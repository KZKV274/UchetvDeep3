(function() {
  ...
  function render() {
    ...
    // Добавляем баннер установки если есть
    if (installBanner.classList.contains('show') && !document.body.contains(installBanner)) {
      app.insertBefore(installBanner, app.firstChild);
    }
    // Вешаем обработчик на кнопку установки
  ...
  function renderSettings() {
    ...
    html += `<hr><div class="install-info">
      ...
      <p>Откройте в Chrome на телефоне, нажмите «⋮» → «Установить приложение» или «На главный экран».</p>
      <p style="font-size:12px;color:#888;margin-top:8px;">Приложение работает полностью офлайн. Все данные хранятся на устройстве.</p>
    </div>`;
